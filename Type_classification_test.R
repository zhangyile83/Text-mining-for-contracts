...
DB <- c("No.", "InfluencerID","OrganizationID", "Total Created Contracts", rep("Words & Aver. Freq.", 20))
Index = 1
NumCount = 0
...
if(is.matrix(TDM1)){
  ...
  TDM3 <- t(TDM3)
  value <- c(Index, 0, UserID, IDFrequency(UserID), colnames(TDM3))
...
value <- value[5:length(value)]
Type <- Type_classifier(value)