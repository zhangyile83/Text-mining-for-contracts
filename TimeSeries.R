...
TimeSeries.User <- function(userID){
  library(RPostgreSQL)
  source("Time_Interval_Generator.R")
  ...
  Interval_FreqCount <- function(Time1, Time2){
    QueryC = paste("SELECT creator_id, created_at FROM contracts WHERE created_at BETWEEN ", Time1, " AND ", Time2, "AND creator_id =" , as.character(userID) , 
                   " ORDER BY created_at")
    drv <- dbDriver("PostgreSQL")
    conn <- dbConnect(drv, dbname = "Inkdit", user = "postgres", password = "314159")
    result_temp <- dbGetQuery(conn, QueryC)
